...
export default function EditProperty() {
    ...
    const [form, setForm] = useState({
        ...
        price_inr: '', size_sqft: '', is_available: 1,
    });
    ...
    useEffect(() => {
        api.get(`/properties/${id}`)
            .then(res => {
                ...
                setForm({
                    property_name: p.property_name || '', property_type: p.property_type || 'Flat',
                    description: p.description || '', location: p.location || '',
                    area: p.area || '', city: p.city || '', state: p.state || '',
                    country: p.country || 'India', price_inr: p.price_inr || '',
                    size_sqft: p.size_sqft || '', is_available: p.is_available,
                });
                // Parse existing extra_details from DB
                let parsed = {};
                if (p.extra_details) {
                    try { parsed = typeof p.extra_details === 'string' ? JSON.parse(p.extra_details) : p.extra_details; }
                    catch { parsed = {}; }
                }
                // Merge with defaults for the property type
                const defaults = EXTRA_DEFAULTS[p.property_type || 'Flat'] || {};
                setExtraDetails({ ...defaults, ...parsed });
            })
            .catch(err => setServerError(err.response?.data?.error || 'Failed to load property'))
    ...
    const handleChange = (e) => {
        ...
        const val = name === 'is_available' ? parseInt(value) : value;
        setForm(prev => ({ ...prev, [name]: val }));

        // When property_type changes, reset extraDetails to defaults for new type
        if (name === 'property_type' && EXTRA_DEFAULTS[value]) {
            setExtraDetails({ ...EXTRA_DEFAULTS[value] });
        }

        if (formSubmitAttempted) setErrors(p => ({ ...p, [name]: validateField(name, val) }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (err.response?.data?.errors) {
                ...
                setErrors(p => ({ ...p, ...se }));
            } else { setServerError(err.response?.data?.error || 'Failed to update property'); }
    ...
    if (loading) {
        return (
            <div className="bg-gray-50 min-h-full flex justify-center items-center py-20">
            ...
            <div className="bg-white border-b border-gray-200">
                <div className="px-6 sm:px-8 py-5">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Edit Property</h1>
                            <p className="text-sm text-gray-400 mt-0.5">Update the property listing details</p>
                        </div>
                        <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ${form.is_available === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                            {form.is_available === 1 ? '🟢 Available' : '🔴 Sold'}
                        </span>
                    </div>
                </div>
            </div>
                        ...
                        <Section title="Property Details" icon="🏗️">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <select name="is_available" className="input-field" value={form.is_available} onChange={handleChange}>
